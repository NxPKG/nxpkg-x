...
describe('@nxpkg/socketio', () => {
  ...
  before((done) => {
    ...
    app = nxpkg()
      .configure(
        socketio((io) => {
          io.use(function (socket: NxpkgSocket, next: NextFunction) {
            socket.nxpkg.user = { name: 'David' }
            socketParams.headers = socket.nxpkg.headers

            const { channel } = socket.handshake.query as any

            if (channel) {
              socket.nxpkg.channel = channel
            }
  ...
  it('can set MaxListeners', (done) => {
    const app = nxpkg().configure(socketio((io) => io.sockets.setMaxListeners(100)))
  ...
  it('can set options (#12)', (done) => {
    const application = nxpkg().configure(
      socketio(
        {
          path: '/test/'
        },
