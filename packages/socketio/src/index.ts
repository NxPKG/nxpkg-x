...
import { Application, RealTimeConnection } from '@nxpkg/nxpkg'
import { socket } from '@nxpkg/transport-commons'

import { disconnect, params, authentication, NxpkgSocket } from './middleware'

const debug = createDebug('@nxpkg/socketio')
...
function configureSocketio(port?: any, options?: any, config?: any) {
  ...
  return (app: Application) => {
    ...
    const getParams = (socket: NxpkgSocket) => socket.nxpkg
    // A mapping from connection to socket instance
    const socketMap = new WeakMap<RealTimeConnection, NxpkgSocket>()
      ...
      Object.assign(app, {
        ...
        async setup(this: any, server: http.Server, ...rest: any[]) {
          if (!this.io) {
            ...
            // In Nxpkg it is easy to hit the standard Node warning limit
