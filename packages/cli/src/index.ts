...
import { generator, runGenerator, getContext, NxpkgBaseContext, version } from '@nxpkg/generators'

export * from 'commander'
export { chalk }

export const commandRunner = (name: string) => async (options: any) => {
  const folder = dirname(require.resolve('@nxpkg/generators'))
  const ctx = getContext<NxpkgBaseContext>({
    ...options
  })
...
program
  .name('nxpkg')
  .description('The Nxpkg command line interface 🕊️')
  .version(version)
  .showHelpAfterError()
