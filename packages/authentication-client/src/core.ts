import { NotAuthenticated, NxpkgError } from '@nxpkg/errors'
import { Application, Params } from '@nxpkg/nxpkg'
import { AuthenticationRequest, AuthenticationResult } from '@nxpkg/authentication'
import { Storage, StorageWrapper } from './storage'

class OauthError extends NxpkgError {
...
export class AuthenticationClient {
  ...
  handleError(error: NxpkgError, type: 'authenticate' | 'logout') {
  ...
  authenticate(authentication?: AuthenticationRequest, params?: Params): Promise<AuthenticationResult> {
    ...
    const promise = this.service
      ...
      .catch((error: NxpkgError) => this.handleError(error, 'authenticate'))
  ...
  logout(): Promise<AuthenticationResult | null> {
    return Promise.resolve(this.app.get('authentication'))
      ...
      .catch((error: NxpkgError) => this.handleError(error, 'logout'))
  }
}
