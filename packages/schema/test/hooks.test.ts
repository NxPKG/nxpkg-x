import { createContext } from '@nxpkg/nxpkg'
import assert from 'assert'
import { app, Message, User } from './fixture'

describe('@nxpkg/schema/hooks', () => {
  const text = 'Hi there'
  let message: Message
  ...
  it('ran resolvers in sequence', async () => {
    assert.strictEqual(user.name, 'hello (nxpkg@khulnasoft.com)')
  })
  ...
  it('resolves safe dispatch data recursively and with arrays and pages', async () => {
    ...
    const user = {
      id: 0,
      email: '[redacted]',
      name: 'hello (nxpkg@khulnasoft.com)'
    }
  ...
  it('validates and converts the query', async () => {
    const otherUser = await app.service('users').create({
      email: 'helloagain@nxpkg.khulnasoft.com',
      password: 'supersecret'
    })
