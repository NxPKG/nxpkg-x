...
import { getContext } from '@nxpkghq/pinion'

import { AppGeneratorContext } from '../src/app'
import { NxpkgBaseContext } from '../src/commons'
...
import { listAllFiles } from '@nxpkghq/pinion/lib/utils'
...
const defaultCombination = {
  language: process.env.NXPKG_LANGUAGE || 'ts',
  framework: process.env.NXPKG_FRAMEWORK || 'koa',
  schema: process.env.NXPKG_SCHEMA || 'typebox'
}

const combinations =
  process.version > 'v16.0.0' ? (process.env.CI ? combinate(matrix as any) : [defaultCombination]) : []

describe('@nxpkg/generators', () => {
  for (const { language, framework, schema } of combinations) {
    describe(`${language} ${framework} app`, () => {
      const name = `nxpkg_${language}_${framework}_${schema}`

      let context: NxpkgBaseContext
      let cwd: string
