...
const template = ({
  ...
  cwd,
  lib
}: ServiceGeneratorContext) => /* ts */ `// For more information about this file see https://dove.nxpkg.khulnasoft.com/guides/cli/service.schemas.html
import { resolve, getValidator, querySyntax } from '@nxpkg/schema'${
  type === 'mongodb'
    ? `
import { ObjectIdSchema } from '@nxpkg/schema'`
    : ''
}
import type { FromSchema } from '@nxpkg/schema'
${localTemplate(authStrategies, `import { passwordHash } from '@nxpkg/authentication-local'`)}
