...
import { generator, runGenerator, runGenerators, prompt } from '@nxpkghq/pinion'
...
export interface ServiceGeneratorContext extends NxpkgBaseContext {
  /**
 ...
 */
export type ServiceGeneratorArguments = NxpkgBaseContext &
  Partial<
  ...
  generator(ctx)
    ...
    .then(
      prompt<ServiceGeneratorArguments, ServiceGeneratorContext>(
        ({ name, path, type, schema, authentication, isEntityService, nxpkg, lib, language }) => {
          ...
          return [
            ...
            },
            {
              ...
              default: getDatabaseAdapter(nxpkg?.database),
              choices: [
                {
                  value: 'knex',
