...
import { generator, runGenerators, prompt, install } from '@nxpkghq/pinion'
import { addVersions, checkPreconditions, NxpkgBaseContext, initializeBaseContext } from '../commons'
...
export type AuthenticationGeneratorArguments = NxpkgBaseContext &
...
export const generate = (ctx: AuthenticationGeneratorArguments) =>
  generator(ctx)
    ...
    .then((ctx) => {
      ...
      dependencies.push('@nxpkg/authentication-oauth')

      if (ctx.authStrategies.includes('local')) {
        dependencies.push('@nxpkg/authentication-local')
      }
