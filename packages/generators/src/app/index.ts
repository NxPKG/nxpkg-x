...
import { generator, prompt, runGenerators, fromFile, install, copyFiles, toFile } from '@nxpkghq/pinion'
import { NxpkgBaseContext, NxpkgAppInfo, initializeBaseContext, addVersions } from '../commons'
import { generate as connectionGenerator, prompts as connectionPrompts } from '../connection'

export interface AppGeneratorData extends NxpkgAppInfo {
  /**
...
export type AppGeneratorContext = NxpkgBaseContext &
  AppGeneratorData & {
    dependencies: string[]
    devDependencies: string[]
  }

export type AppGeneratorArguments = NxpkgBaseContext & Partial<AppGeneratorData>
      ...
      install<AppGeneratorContext>(
        ({ transports, framework, dependencyVersions, dependencies, schema }) => {
          ...
          dependencies.push(
            ...
            '@nxpkg/schema',
            '@nxpkg/configuration',
            '@nxpkg/transport-commons',
            '@nxpkg/adapter-commons',
            '@nxpkg/authentication',
            '@nxpkg/authentication-client',
            'winston'
          )

          if (hasSocketio) {
            dependencies.push('@nxpkg/socketio')
          }

          if (framework === 'koa') {
            dependencies.push('@nxpkg/koa')
          }

          if (framework === 'express') {
            dependencies.push('@nxpkg/express', 'compression')
          }

          if (schema === 'typebox') {
            dependencies.push('@nxpkg/typebox')
          }
