...
/**
 * Returns the name of the Nxpkg database adapter for a supported database type
 *
...
export interface NxpkgBaseContext extends PinionContext {
  /**
   * Information about the Nxpkg application (like chosen language, database etc.)
 ...
 * and Nxpkg app specific information.
 *
 * @returns The updated context
 */
export const initializeBaseContext =
  () =>
  <C extends NxpkgBaseContext>(ctx: C) =>
    Promise.resolve(ctx)
      ...
      .then((ctx) => ({
        ...
        language: ctx.language || ctx.pkg?.nxpkg?.language,
        nxpkg: ctx.pkg?.nxpkg
      }))
...
export const checkPreconditions =
  () =>
  async <T extends NxpkgBaseContext>(ctx: T) => {
    if (!ctx.nxpkg) {
      throw new Error(`Can not run generator since the current folder does not appear to be a Nxpkg application.
Either your package.json is missing or it does not have \`nxpkg\` property.
`)
    }
