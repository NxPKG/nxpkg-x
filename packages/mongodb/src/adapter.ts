...
import { BadRequest, MethodNotAllowed, NotFound } from '@nxpkg/errors'
...
import {
  ...
  AdapterQuery,
  getLimit
} from '@nxpkg/adapter-commons'
import { Id, Paginated } from '@nxpkg/nxpkg'
...
> extends AdapterBase<Result, Data, PatchData, ServiceParams, MongoDBAdapterOptions, AdapterId> {
  ...
  async aggregateRaw(params: ServiceParams) {
    ...
    const index = pipeline.findIndex((stage: Document) => stage.$nxpkg)
    const before = index >= 0 ? pipeline.slice(0, index) : []
    const nxpkgPipeline = this.makeNxpkgPipeline(params)
    const after = index >= 0 ? pipeline.slice(index + 1) : pipeline

    return model.aggregate([...before, ...nxpkgPipeline, ...after])
  }

  makeNxpkgPipeline(params: ServiceParams) {
    const { filters, query } = this.filterQuery(null, params)
