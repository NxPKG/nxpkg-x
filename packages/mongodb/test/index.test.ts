...
import adapterTests from '@nxpkg/adapter-tests'
import assert from 'assert'
import { MongoMemoryServer } from 'mongodb-memory-server'
import { Ajv, FromSchema, getValidator, hooks, querySyntax } from '@nxpkg/schema'
...
describe('Nxpkg MongoDB Service', () => {
  const personSchema = {
    $id: 'Person',
    type: 'object',
  ...
  describe('Aggregation', () => {
    ...
    it('assumes the nxpkg stage runs before all if it is not explicitly provided in pipeline', async () => {
    ...
    it('can prepend stages by explicitly placing the nxpkg stage', async () => {
      const result = await app.service('todos').find({
        query: { $sort: { name: 1 } },
        pipeline: [
