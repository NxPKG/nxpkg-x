...
import { NotAcceptable } from '@nxpkg/errors'
...
describe('Axios REST connector', function () {
  ...
  it('converts nxpkg errors (#50)', async () => {
    try {
      await service.get(0, { query: { nxpkgError: true } })
      assert.fail('Should never get here')
    } catch (error: any) {
      assert.ok(error instanceof NotAcceptable)
      assert.strictEqual(error.message, 'This is a Nxpkg error')
  ...
  it('ECONNREFUSED errors are serializable', async () => {
    ...
    const app = nxpkg().configure(setup)
