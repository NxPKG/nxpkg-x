...
import { MethodNotAllowed } from '@nxpkg/errors'
import { createDebug } from '@nxpkg.khulnasoft.commons'
import { http } from '@nxpkg/transport-commons'
import { createContext, defaultServiceMethods, getServiceOptions } from '@nxpkg/nxpkg'

import { AuthenticationSettings, parseAuthentication } from './authentication'
import { Application } from './declarations'

const debug = createDebug('@nxpkg/express/rest')
...
const serviceMiddleware = (): RequestHandler => {
  return toHandler(async (req, res, next) => {
    ...
    const params = { query, headers, route, ...req.nxpkg }
...
export const rest = (options?: RestOptions | RequestHandler) => {
  ...
  return (app: Application) => {
    if (typeof app.route !== 'function') {
      throw new Error('@nxpkg/express/rest needs an Express compatible app.')
    }
