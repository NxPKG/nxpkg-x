...
import { createApplication } from '@nxpkg/authentication-local/test/fixture'
import { authenticate, AuthenticationResult } from '@nxpkg/authentication'
...
describe('@nxpkg/express/authentication', () => {
  ...
  before(async () => {
    const expressApp = expressify(nxpkg()).use(express.json()).configure(express.rest())
