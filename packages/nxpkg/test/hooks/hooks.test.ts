...
import { hooks, NextFunction } from '@nxpkg/hooks'
import { HookContext, createContext, nxpkg, Id, Params, ServiceInterface } from '../../src'

describe('hooks basics', () => {
  it('mix @nxpkg/hooks and .hooks', async () => {
    ...
    const app = nxpkg().use('/dummy', new SimpleService())
  ...
  it('works with services that return a promise (nxpkg-hooks#28)', async () => {
    interface DummyParams extends Params {
      user: string
    }

    const app = nxpkg<{ dummy: ServiceInterface<any, any, DummyParams> }>().use('dummy', {
      async get(id: any, params: any) {
        return { id, user: params.user }
      }
    })
  ...
  describe('returns the context when passing it as last parameter', () => {
    ...
    // it('on argument validation error (https://github.com/nxpkg/express/issues/19)', async () => {
    //   const app = nxpkg().use('/dummy', {
    //     async get (id: string) {
    //       return { id };
    //     }
    ...
    // it('on error in error hook (https://github.com/nxpkg/express/issues/21)', async () => {
    //   const app = nxpkg().use('/dummy', {
    //     async get () {
