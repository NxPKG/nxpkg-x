...
import { nxpkg, Params, ServiceInterface } from '../../src'
...
describe('`around` hooks', () => {
  ...
  it('contains the app object at hook.app', async () => {
    ...
    const app = nxpkg<{ 'some-service': typeof someServiceConfig }>().use(
      'some-service',
      someServiceConfig
    )
  ...
  it('adds .hooks() and chains multiple hooks for the same method', async () => {
    ...
    const app = nxpkg<{ dummy: DummyService }>().use('dummy', new DummyService())
    const service = app.service('dummy')
  ...
  it('around all hooks (#11)', async () => {
    ...
    const app = nxpkg<{ dummy: DummyService }>().use('dummy', {
  ...
  it('around hooks have service as context and keep it in service method (#17)', async () => {
    ...
    const app = nxpkg<{ dummy: Dummy }>().use('dummy', new Dummy())
