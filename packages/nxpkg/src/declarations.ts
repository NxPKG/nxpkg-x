...
import { NextFunction, HookContext as BaseHookContext } from '@nxpkg/hooks'
...
export type NxpkgService<A = NxpkgApplication, S = Service> = S &
  ServiceAddons<A, S> &
...
export type ServiceMixin<A> = (service: NxpkgService<A>, path: string, options: ServiceOptions) => void
...
export interface NxpkgApplication<Services = any, Settings = any> {
  /**
   * The Nxpkg application version
   */
  version: string
   ...
   * Nxpkg application, all its services will be re-registered
   ...
   * Nxpkg application to use a sub-app under the `path` prefix.
  ...
  unuse<L extends keyof Services & string>(
    path: L
  ): Promise<NxpkgService<this, keyof any extends keyof Services ? Service : Services[L]>>

  /**
   * Get the Nxpkg service instance for a path. This will
   * be the service originally registered with Nxpkg functionality
  ...
  service<L extends keyof Services & string>(
    path: L
  ): NxpkgService<this, keyof any extends keyof Services ? Service : Services[L]>
...
export interface Application<Services = any, Settings = any>
  extends NxpkgApplication<Services, Settings>,
    EventEmitter {}
...
export interface HookContext<A = Application, S = any> extends BaseHookContext<ServiceGenericType<S>> {
  /**
   * A read only property that contains the Nxpkg application object. This can be used to
