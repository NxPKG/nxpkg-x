...
export class NxpkgHookManager<A> extends HookManager {
  constructor(
    public app: A,
...
export function hookMixin<A>(this: A, service: NxpkgService<A>, path: string, options: ServiceOptions) {
  ...
  const serviceMethodHooks = hookMethods.reduce((res, method) => {
    ...
    res[method] = new NxpkgHookManager<A>(this, method).params(...params).props({
      app: this,
      path,
      method,
      service,
  ...
  service.hooks = function (this: any, hookOptions: any) {
    ...
    Object.keys(hookOptions).forEach((method) => {
      ...
      if (!(manager instanceof NxpkgHookManager)) {
        throw new Error(`Method ${method} is not a Nxpkg hooks enabled service method`)
      }
