...
import { ApplicationHookMap, nxpkg, Id } from '@nxpkg/nxpkg'
...
describe('@nxpkg/koa', () => {
  ...
  it('throws an error when initialized with invalid application', () => {
    try {
      ...
      assert.equal(error.message, '@nxpkg/koa requires a valid Nxpkg application instance')
    }
  })

  it('returns Koa instance when no Nxpkg app is passed', () => {
  ...
  it('starts as a Koa and Nxpkg application', async () => {
