...
describe('@nxpkg/authentication-oauth/strategy', () => {
  ...
  it('getRedirect with referrer and allowed origins (#2430)', async () => {
    app.get('authentication').oauth.origins = ['https://nxpkg.khulnasoft.com', 'https://nxpkg.cloud']

    let redirect = await strategy.getRedirect(
      { accessToken: 'testing' },
      {
        headers: {
          referer: 'https://nxpkg.khulnasoft.com/somewhere'
        }
      }
    )
    assert.equal(redirect, 'https://nxpkg.khulnasoft.com#access_token=testing')
    ...
    redirect = await strategy.getRedirect(
      ...
      {
        headers: {
          referer: 'HTTPS://nxpkg.CLOUD'
        }
      }
    )
    assert.equal(redirect, 'https://nxpkg.cloud#access_token=testing')
    ...
    assert.equal(redirect, 'https://nxpkg.khulnasoft.com/home#access_token=testing')
