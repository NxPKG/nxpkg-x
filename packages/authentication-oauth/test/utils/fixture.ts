import { Application, nxpkg, NextFunction } from '@nxpkg/nxpkg'
import express, { rest, errorHandler } from '@nxpkg/express'
...
export const expressFixture = async (serverPort: number, providerPort: number) => {
  ...
  const app = express<ServiceTypes>(nxpkg())
  ...
  app.use((req, _res, next) => {
    req.nxpkg = { fromMiddleware: 'testing' }
    next()
  })
