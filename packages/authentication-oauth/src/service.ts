...
import { HookContext, NextFunction, Params } from '@nxpkg/nxpkg'
import { NxpkgError, GeneralError } from '@nxpkg/errors'
...
const debug = createDebug('@nxpkg/authentication-oauth/services')
...
export class OAuthError extends NxpkgError {
  constructor(
    message: string,
    data: any,
...
export class OAuthService {
  ...
  async find(params: OAuthParams) {
    ...
    const { nxpkg_token, redirect, ...restQuery } = query
    const handlerParams = {
      ...params,
      query: restQuery
    }
    ...
    if (nxpkg_token) {
      debug('Got nxpkg_token query parameter to link accounts', nxpkg_token)
      session.accessToken = nxpkg_token
    }
