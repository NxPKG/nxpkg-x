...
import { Ajv, schema } from '@nxpkg/schema'
import configuration from '../src'

describe('@nxpkg/configuration', () => {
  const app: Application = nxpkg().configure(configuration())
  ...
  it('errors on .setup when a schema is passed and the configuration is invalid', async () => {
    ...
    const schemaApp = nxpkg().configure(configuration(configurationSchema))
